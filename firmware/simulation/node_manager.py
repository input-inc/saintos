...
class NodeManager:
    ...
    def __init__(self):
        ...
        # platformio.ini sets `build_dir = build` to surface artifacts at
        # firmware/teensy41/build/<env>/ instead of the default
        # .pio/build/<env>/ — match that here.
        self.teensy41_build_dir = self.teensy41_dir / "build" / "simulation"
    ...
    def _generate_rp2040_resc(self, node_id: str, udp_port: int) -> str:
        ...
        uart_log_path = self.logs_dir / f"{node_id}.uart.log"
...
# stream had nowhere to land and operators saw an empty log. The
# CreateFileBackend tap mirrors UART0 bytes to a text file alongside
# the Renode operational log; `node_manager.py logs <id>` (and the
# e2e test harness) reads this file for firmware boot markers.
...
'''
    ...
    def _generate_teensy41_resc(self, node_id: str, udp_port: int) -> str:
        ...
        # SAINT_TEENSY_TRACE=1 enables LoadSymbolsFrom + LogFunctionNames +
        # verbose lpuart6 register logging so the Renode operational log
        # shows both function entry boundaries and every UART read/write.
        # Off by default because the per-call symbol resolution adds
        # noticeable overhead. Do NOT pair with `cpu MaximumBlockSize 1` —
        # see feedback_renode_invocation_traps.md.
        diagnostics_block = ""
        if os.environ.get("SAINT_TEENSY_TRACE") == "1":
            diagnostics_block = f'''
# Diagnostics — function-level tracing (SAINT_TEENSY_TRACE=1).
sysbus LoadSymbolsFrom @{firmware_path}
cpu LogFunctionNames true
logLevel -1 sysbus.lpuart6
'''
...
# Load Teensy 4.1 platform (inherits Renode's imxrt1064.repl + adds the
# SAINT-specific external_flash, udp_bridge, and persistent_storage).
...
# Load firmware into the external-flash region (0x60000000 — XIP base).
sysbus LoadELF @{firmware_path}

# Teensy 4.1 startup, in real life:
#   bootrom @0x00200000 → reads FlexSPI Config Block @0x60000000 → reads
#   IVT @0x60001000 → IVT[entry] = 0x6000164D (the application's
#   ResetHandler) → bootrom jumps there. ResetHandler then copies
#   .text.itcm from flash to ITCM @0x0, writes VTOR, calls main().
# We can't run the bootrom in the sim (no model), so jump straight to
# the ResetHandler. Initial SP can be anywhere valid — the Teensy
# startup code reinitializes it before any real stack use. DTCM at
# 0x20080000 (top of DTCM, grows down) is the natural choice.
sysbus.cpu PC 0x6000164D
sysbus.cpu SP 0x20080000

# Capture firmware UART output to a dedicated file. The firmware redirects
# `Serial` → `Serial1` under SIMULATION (see firmware/teensy41/include/
# platform.h); Serial1 on Teensy 4.1 = LPUART6. Renode's NXP_LPUART model
# at 0x40198000 maps to `lpuart6` in the imxrt1064 repl. CreateFileBackend
# mirrors that peripheral's TX bytes to a text file the harness can tail.
sysbus.lpuart6 CreateFileBackend @{uart_log_path} true
...
'''
    ...
    def _start_teensy41(self, node_id: str, node: Dict, foreground: bool = False) -> bool:
        ...
        # TTY; in background mode it pairs badly with stdin=DEVNULL because
        # Renode's monitor reads EOF immediately and disposes the machine.
        # (Same gotcha as the RP2040 path — see node_manager.py:_start_rp2040.)
        base_cmd = [self.renode_path, "--disable-xwt"]
