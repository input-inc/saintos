/**
 * SAINT.OS Firmware - SyRen 50 Motor Controller Driver (shared)
 *
 * Drives up to 8 Dimension Engineering SyRen 50 motor controllers
 * over packetized serial (addresses 128-135).
 *
 * Platform-agnostic: the protocol byte assembly + channel state + the
 * peripheral_driver_t glue live in shared/src/syren_driver.c. Each
 * platform's UART adapter implements syren_transport_ops (open/write)
 * — see shared/include/syren_transport.h. Mirrors the architecture of
 * the Maestro driver (shared core + per-platform transport).
 */

#ifndef SAINT_SYREN_DRIVER_H
#define SAINT_SYREN_DRIVER_H
