...
`/opt/saint-os/saint_os/resources/firmware/<type>/`. The three
...
SAINT.OS server via mDNS. See `saint_os/firmware/rpi5/` for details
...
2. Pick a **role** (e.g. `tracks`, `head`, `arm_left`) — roles come
   ...
   `/opt/saint-os/saint_os/saint_server/roles/` and are what the
...
| `/opt/saint-os/saint_os/resources/firmware/` | Node firmware artifacts served at `/api/firmware/` |
