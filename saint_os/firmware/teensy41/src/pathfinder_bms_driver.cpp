 ...
 *
 * Uses HardwareSerial (Serial4) for JBD UART communication at 9600 baud.
    ...
    switch (rx_state) {
        ...
        case RX_WAIT_LENGTH:
            ...
            if (rx_length == 0) {
                ...
                rx_state = RX_IDLE;  // Invalid — reset
            } else {
...
void pathfinder_bms_update(void)
{
    ...
    if (now - last_poll_time >= poll_interval_ms) {
        last_poll_time = now;
        rx_state = RX_IDLE;  // Reset state machine for new response
...
{
    Serial.printf("Pathfinder BMS: ESTOP (no action needed for read-only sensor)\n");
}
