 ...
 *
 * Uses HardwareSerial (default Serial2) for packetized serial communication
 * with up to 8 SyRen 50 motor controllers on addresses 128-135.
 */
...
void syren_set_channel_config(uint8_t channel, const syren_channel_config_t* config)
{
    ...
    // Apply deadband and ramping to the device if port is ready
    if (port_initialized) {
#ifndef SIMULATION
        ...
        if (config->timeout_ms > 0) {
            // Timeout value is in 100ms increments for the SyRen
...
/**
 * set_value: float -1.0..+1.0 mapped to int -127..+127
 */
...
{
    // SyRen has no position readback — return false
    (void)channel;
    (void)value;
    return false;
}
...
static bool syren_drv_parse_json(const char* json_start, const char* json_end,
                                  pin_config_t* config)
{
    // Parse SyRen-specific JSON parameters
    const char* p;
...
static bool syren_drv_load(const void* storage_ptr)
{
    ...
    for (uint8_t ch = 0; ch < count; ch++) {
        ...
        // Only apply if address is valid (i.e., was actually configured)
        if (channel_configs[ch].address >= SYREN_ADDRESS_MIN &&
            channel_configs[ch].address <= SYREN_ADDRESS_MAX) {
            // Config will be applied on next init or apply_config call
        }
    }

    Serial.printf("SyRen: restored %d channel configs from flash\n", count);
    return true;
}
