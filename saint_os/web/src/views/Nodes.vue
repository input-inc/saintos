...
<template>
  <section>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold text-white">Nodes</h2>
      <div class="flex items-center gap-2">
        <span class="text-xs text-slate-400">{{ nodes.all.length }} adopted · {{ nodes.unadopted.length }} unadopted</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="n in nodes.unadopted" :key="n.node_id" class="node-card border-amber-700/40 flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <span class="font-semibold text-white">{{ n.node_id }}</span>
          ...
          <div class="flex gap-2 mt-1">
            ...
            <button class="btn-sm bg-slate-700 hover:bg-red-600 text-slate-300 hover:text-white" @click="remove(n.node_id)">
    ...
    <div v-if="nodes.loading && !nodes.all.length" class="text-sm text-slate-400">Loading…</div>

    <div v-else-if="!nodes.all.length" class="card text-center py-10">
      <span class="material-icons icon-lg text-slate-600">dns</span>
      <h3 class="text-lg font-semibold text-slate-300 mt-4">No nodes adopted yet</h3>
      <p class="text-slate-500 text-sm mt-1">Scan to discover available nodes.</p>
    </div>
      ...
      >
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="font-semibold text-white">{{ n.display_name || n.node_id }}</span>
          </div>
